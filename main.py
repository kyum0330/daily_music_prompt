...
- chill, lo-fi, soft instrumentation, minimalist 
    ...
    1-4. 고음이 들어가는 부분에서는 단어 사이 사이에 ',', '.'를 삽입하여 의도적으로 숨을 고르게 만들게 합니다. 이때, 너무 과하게 사용하지 않고, 필요한 부분에만 적절하게 사용합니다.
...
###TAG###
이 곡과 어울리는 유튜브 노출용 트렌디 해쉬태그를 이용해서 한글과 영어 섞어서 정확히 30개 작성해줘요. 이때 #표시는 하지말고, ','로 구분만 하여 적어주세요.

###UPLOAD###
유튜브 업로드용 요약 양식으로 작성해주세요. 양삭은 하단의 예시를 참고해서 적어주세요. 유튜브 노출용 해쉬태그 5개, 작사가의 한마디, 그리고 곡 정보를 적어주세요. 
이때 해쉬태그를 작성할때는 제목의 내용은 작성하지 말고, 한영으로 중복되는 것도 제외하고, 유튜브 노출에 최우선되는 항목으로 부탁해요.

#playlist #감성 #ReggaeCountry #ChillVibes #LoFiMusic
