...
###LYRICS### 섹션을 작성할 때, 가사 텍스트만 적지 말고 반드시 아래의 3가지 요소를 < > (악기/효과음/구조)와 <> (보컬 창법) 기호를 사용하여 영어로 표현해서 촘촘하게 배치해주세요. 이때 3가지 요소와 가사의 총 글자수는 4900자를 넘어가지 않도록 해주세요.
...
###SUNO###
위 DETAIL 부분에 작성한 '장르, Tempo, 악기 구성, 분위기'를 음악 생성 AI(Suno)의 'Style of Music' 란에 바로 복사해 넣을 수 있도록, 영어 키워드 위주로 500~700자로 번역 및 요약해주세요.
...
###TAG###
이 곡과 어울리는 유튜브 노출용 트렌디 해쉬태그를 이용해서 한글과 영어 섞어서 정확히 30개 작성해줘요. 이때 번갈아가며 나오도록 하고, 해당 태크마다','를 붙여주고, 노출 가능성이 큰 순서대로 나열해주세요.
...
"""
    ...
    for model_name in preferred_models:
        try:
            ...
            # 🌟 강력한 정규표현식 파싱 로직 (반드시 try 문법 안에 있어야 함)
    ...
    # 모든 모델이 실패했을 경우
    print("🚨 모든 모델에서 생성을 실패했습니다. (할당량 소진 또는 통신 오류)")
    return {"suno": "", "lyrics": ""} 
...
"""
    ...
    print("\n[3] Notion 저장 시도...")
    # 🚨 여기에 빠져있던 current_weather(날씨 변수)를 추가하여 5개 매개변수 기준을 맞췄습니다!
